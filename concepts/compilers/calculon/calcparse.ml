...
and parse_or toks = 
  ...
  match rest with
  | OrTok :: tail ->
    let (rexpr, tail) = parse_and tail in
    (Boolop{op=Or;lexpr;rexpr}, tail)
  | _ -> (lexpr, rest)
...
and parse_and toks =
  ...
  match rest with
  | AndTok :: tail ->
    let (rexpr, tail) = parse_compare tail in
    (Boolop{op=And;lexpr;rexpr}, tail)
  | _ -> (lexpr, rest)
...
and parse_compare toks =
  ...
  match rest with
  | LessThan :: tail ->
    let (rexpr, tail) = parse_addsub tail in
    (Intop{op=Less;lexpr;rexpr}, tail)
  | GreatThan :: tail ->
    let (rexpr, tail) = parse_addsub tail in
    (Intop{op=Greater;lexpr;rexpr}, tail)
  | Equal :: tail ->
    let (rexpr, tail) = parse_addsub tail in
    (Intop{op=Equal;lexpr;rexpr}, tail)
  | _ -> (lexpr, rest)
...
and parse_cond toks =
  match toks with
  | If :: rest ->                                     (* look for a sequence of 'if test then.. else..' *)
    (let (if_expr, rest) = parse_expr rest in         (* parse the if_expr *)
    match rest with
    | Then :: rest ->                                 (* check for a then keyword *)
      (let (then_expr, rest) = parse_expr rest in     (* parse the then_expr *)
      match rest with
      | Else :: rest ->                               (* check for an else keyword *)
        let (else_expr, rest) = parse_expr rest in    (* parse the else_expr *)
