...
and eval_statement tokens =
  ...
  | [Ident "source"; Ident filename; Semicolon] ->
    let chan = open_in filename in (* Open file for reading *)
    printf "Reading source file '%s'\n" filename;
    begin
      try
        while true do
          process_statement chan;
          let line  = input_line chan in  (* Read line from in_channel and discard \n *)
      ...
      | End_of_file ->                    (* End of file reached *)
        ...
        close_in chan                     (* close the input channel *)
      | e ->
        close_in_noerr chan;              (* emergency closing *)
        raise e
      end
